...
export default function PushDayPage() {
  ...
  useEffect(() => {
    // Filter ChestTriceps exercises and organize by variation
    const chestTricepsExercises = (exercisesData as Exercise[]).filter(
      exercise => exercise.category === 'ChestTriceps'
    )
    setExercises(chestTricepsExercises)
  }, [])

  // Organize exercises by variation
  const pushAExercises = exercises.filter(ex => ex.variation === 'A')
  const pushBExercises = exercises.filter(ex => ex.variation === 'B') 
    ...
    <div className="min-h-screen bg-fitbod-background text-fitbod-text p-4">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex items-center gap-4 mb-6">
          ...
          <div>
            <h1 className="text-2xl font-bold text-fitbod-text">Build Your Push Workout</h1>
            <p className="text-fitbod-text-secondary">
              Add exercises from Push A, Push B, or shared exercises
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-6">
          ...
          <Card className="bg-fitbod-card border-fitbod-subtle">
            <CardHeader className="pb-3">
              <CardTitle className="text-lg text-fitbod-text">Push A</CardTitle>
            </CardHeader>
            <CardContent className="space-y-2">
              {pushAExercises.map((exercise) => (
          ...
          <Card className="bg-fitbod-card border-fitbod-subtle">
            <CardHeader className="pb-3">
              <CardTitle className="text-lg text-fitbod-text">Push B</CardTitle>
            </CardHeader>
            <CardContent className="space-y-2">
              {pushBExercises.map((exercise) => (
                <div key={exercise.id} className="flex items-center justify-between p-2 hover:bg-fitbod-subtle rounded">
                  <span className="text-sm text-fitbod-text">{exercise.name}</span>
                  <Button
                    size="sm"

...
export default function LegsDayPage() {
  ...
  useEffect(() => {
    // Filter Legs exercises and organize by variation
    const legsExercises = (exercisesData as Exercise[]).filter(
      exercise => exercise.category === 'Legs'
    )
    setExercises(legsExercises)
  }, [])

  // Organize exercises by variation
  const legsAExercises = exercises.filter(ex => ex.variation === 'A')
  const legsBExercises = exercises.filter(ex => ex.variation === 'B') 
    ...
    <div className="min-h-screen bg-fitbod-background text-fitbod-text p-4">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex items-center gap-4 mb-6">
          ...
          <div>
            <h1 className="text-2xl font-bold text-fitbod-text">Build Your Legs Workout</h1>
            <p className="text-fitbod-text-secondary">
              Add exercises from Legs A, Legs B, or shared exercises
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-6">
          ...
          <Card className="bg-fitbod-card border-fitbod-subtle">
            <CardHeader className="pb-3">
              <CardTitle className="text-lg text-fitbod-text">Legs A</CardTitle>
            </CardHeader>
            <CardContent className="space-y-2">
              {legsAExercises.map((exercise) => (
          ...
          <Card className="bg-fitbod-card border-fitbod-subtle">
            <CardHeader className="pb-3">
              <CardTitle className="text-lg text-fitbod-text">Legs B</CardTitle>
            </CardHeader>
            <CardContent className="space-y-2">
              {legsBExercises.map((exercise) => (
                <div key={exercise.id} className="flex items-center justify-between p-2 hover:bg-fitbod-subtle rounded">
                  <span className="text-sm text-fitbod-text">{exercise.name}</span>
                  <Button
                    size="sm"
                      ...
                      <div className="flex items-center gap-2 mb-3 p-2 bg-fitbod-background rounded">
                        ...
                        <div className="flex items-center gap-1">
                          <Button
                            size="sm"
                            variant="ghost"
                            onClick={() => updateExerciseRestTime(exercise.id, (exercise.plannedSets[0]?.restSeconds || 180) - 30)}
                          ...
                          <Input
                            type="number"
                            value={exercise.plannedSets[0]?.restSeconds || 180}
                            onChange={(e) => updateExerciseRestTime(exercise.id, parseInt(e.target.value) || 180)}
                            ...
                            {exercise.equipment !== 'Bodyweight' && (
                              <div className="flex items-center gap-1">
                                <Button
                                  size="sm"
                                  variant="ghost"
                                  onClick={() => updateSetValue(exercise.id, setIndex, 'targetWeight', set.targetWeight - 10)}
                                  className="h-6 w-6 p-0 text-fitbod-text-secondary hover:text-fitbod-text"
                                >
                                  <Minus className="h-2 w-2" />
                                </Button>
